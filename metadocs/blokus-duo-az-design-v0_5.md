...
- **M1 — Blokus rules engine:** slow cell-grid oracle **first**, then the bitboard engine, **independently implemented**. Test battery:
  ...
  - Encode/decode bijection over all in-bounds actions. **Ownership boundary (resolves the M1/M2 overlap):** M1 owns `encode_action`/`decode_action` **and** the geometric `(g, a) → a′` symmetry-permutation table; M2 owns the 46-plane state encoding and the training-side wiring of symmetry into targets/augmentation.
