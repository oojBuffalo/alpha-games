...
**Status:** v0.5 — decision-complete; milestone plan re-scoped. The pinned decisions **D1–D12** are unchanged and the success criterion (§1) stays pre-registered; §11 holds only an optional stretch goal. Three previously-unstated config *scalars* — the aux loss weight `λ_aux` (§7), the weight-publish interval / checkpoint count *K* (§6.2/§1), and the mirrored game-pairs per (checkpoint, rung) cell (§9) — are **not** among D1–D12; all three are now pinned: `λ_aux` at M2 (§7: 0.25), the weight-publish interval / checkpoint count `K` at M3 (§6.2: interval = 200 learner steps, `K` = 30), and the mirrored game-pairs per cell at M4 (§9: **24 pairs, 48 games, per cell** — landed together with the full pre-registered §1/§9 evaluation protocol: cell semantics, the eval search-form + sim budget, the rung-8 selection rule, the finite-fit convention, and the bootstrap/Mann–Kendall statistical conventions). Buildable as written.
...
**Changes within v0.5 (M1.5 convention pins):** §12 M1.5 gains a "Convention pins (M1.5, Othello)" block freezing the operational Othello conventions its adapter, encoding surface, and symmetry table depend on — 0-indexed coordinates, flatten `a = r*8 + c` with pass = 64 and flat head `(65,)`, player 0 = Black moving first from the standard initial position, pass-legality/termination rule, the 2 mover-relative input planes (`T=1`), the D4 element order with rot90 = clockwise, and `z = sign(disc_diff)` with no aux head. Unlike Blokus (whose plane slot is an M2 sentinel), M1.5 carries its own encoding, so Othello's `symmetry_group` plane-transform slot is *real* at M1.5. No decision changes; D1–D12 untouched.
...
**Changes within v0.5 (M2 contract addition):** §6.1's core contract gains `input_shape` — the declared `(height, width)` of every input plane, making the full input tensor `(input_planes, *input_shape)`. Declared rather than derived because a flat policy head (Othello's `(65,)`) carries no board geometry to recover `(H, W)` from; the M2 network build needs a stable geometry source per game. Implemented by Blokus (`(14, 14)`) and backfilled on Othello (`(8, 8)`, whose M1.5-carried encoding predates the property). No decision changes; D1–D12 untouched.
**Changes within v0.5 (M4 protocol pins):** §9 gains a "Pre-registered protocol (M4 pins)" block of eleven numbered pins completing the full §1/§9 evaluation protocol before any evaluation game exists — the mirrored game-pairs per (checkpoint, rung) cell (24 pairs, 48 games; the last of the three flagged scalars), the checkpoint-parameterized cell semantics, the eval search-form definition and its S = 512 sim budget, the rung-8 historical-opponent rule, the finite-fit convention (retaining §12 M1.6's virtual-draw regularizer), the bootstrap/Mann–Kendall statistical conventions (the admissible-`B` rank rule, `bootstrap_seed` derivation, and pinned degenerate cases), the provisional-vs-authoritative reporting rule, the analysis-snapshot definition, the protocol-version/fingerprint mechanism, and the operational liveness bound. §12 M1.6's "supersedes" is amended to "adopts" accordingly (pin 6). No decision changes; D1–D12 untouched.
...
policy_shape ; input_planes ; input_shape
    # input_shape = (height, width) of every input plane; the full input tensor is
    # (input_planes, *input_shape). Declared, not derived: a flat policy head
    # (Othello's (65,)) carries no board geometry to recover (H, W) from. (M2 amendment;
    # Blokus (14, 14), Othello backfilled (8, 8), Connect 4's would be the non-square (6, 7).)
```
...
- **Version pinning:** publish weights every **200 learner steps**; pin one model version per self-play game; never swap mid-search; record `model_version` per replay sample; reuse the chosen subtree between moves. **Publish interval = 200 learner steps; checkpoint count `K` = 30** for the primary Δ contrast (§1) — pinned doc-first at M3, per the §12 flag. `model_version` provenance is distinct from the §5.1 orientation-table hash.
  - *Arithmetic (M2.5's measured 151.2 games/hour GO record, issue #66; D5 pacing, §7):* self-play stores ≈9–10 positions/game; at D5's replay ratio 2–4 samples/stored position and batch 256, steady-state learner pacing is `games/step = batch / (positions_per_game · ratio)` ≈ 6.4–14.2 games per learner step (point estimate ≈9, at ratio 3 / 9.5 positions/game — cross-checked directly against the measured rates: `3600 s / 151.2 games·h⁻¹ ≈ 23.8 s/game` vs. `3600 s / 16.8 steps·h⁻¹ ≈ 214 s/step`, the same ≈9 ratio). Self-play so far outpaces the learner (§3's binding-constraint claim, confirmed live by issue #66: self-play 96.3% of GPU time vs. learner 3.4%) that *any* interval of even a few steps already spans many whole games under one pinned version: 200 steps ≈ 1,796 games ≈ 11.9 hours between publishes at 151.2 games/hour, clearing the "plays whole games" floor with wide margin — the binding constraint on the interval is the production-budget ceiling below, not this floor. `total_steps = K · publish_interval = 30 × 200 = 6,000`; at the measured pre-M5 rate (`games/hour · positions/game · ratio / batch ≈ 16.8 steps/hour`), that is `6,000 / 16.8 ≈ 357` GPU-hours (≈15 days) — a plausible multi-day single-GPU production run (M5's batched inference + PCR only shortens it, never lengthens it — 151.2 games/hour is the pre-batching floor, not the production rate). `K = 30` sits centrally in the 20–40 band so both the first/final-third averaging and the Mann–Kendall trend (§1) stay meaningful even if a handful of checkpoints are later excluded.
  - *Checkpoint/publish boundary (pinned):* **(a)** every publish is a checkpoint — no publish without a checkpoint, no checkpoint outside a publish. **(b)** the seeded-init network (`v0`, pre-training) is a recorded artifact, not a `K`-set member — checkpoint 1 is the first post-training publish, at step 200. **(c)** exact stop arithmetic, no partial final interval: the run stops at `total_steps = K · publish_interval` exactly (step 6,000), never rounding to finish an in-progress interval. **(d)** crash-resume snapshots (§12 M3 checkpoint schema) are saved for restart safety but are never `K`-set members — only the fixed-`publish_interval` publishes count toward `K`.
...
- **Loss:** `l = (z − v)² − πᵀ log p + λ_aux·(aux term) + c‖θ‖²`. The aux-term weight is **pinned at `λ_aux = 0.25`** (M2 amendment, per the §12 flag): the KataGo lineage makes the score-diff head auxiliary, not co-primary — the aux MSE has targets in [−1, 1] on the same scale as the value MSE, so 0.25 keeps it a clear minority of the value-side gradient while still shaping the trunk. It remains a config scalar (not a D-decision); retune only on M6 evidence.
...
**Protocol:** disable Dirichlet; deterministic root (argmax N); alternate first player and balance the start-point choice; **paired/mirrored games** — the resampling unit for §1's bootstrap (the number of mirrored pairs per (checkpoint, rung) cell governs CI width, distinct from §1's B ≈ 2,000 bootstrap replicates; pinned below at **24 pairs, 48 games, per cell** — pin 1); draws scored 0.5; report per-checkpoint confidence intervals; freeze baseline versions. The pre-registered Δ contrast (§1) is the primary criterion; the Mann–Kendall test is the secondary; per-checkpoint monotonicity is not required. (For the future N-player engine, 2p Elo is replaced by tournament / per-seat scoring — see M7.)

**Pre-registered protocol (M4 pins).** The §1 primary criterion cannot be computed without the following operational conventions. All eleven are pinned here, doc-first, before any evaluation game exists, and every downstream M4 task implements against exactly §9's eleven pins.

1. **Pairs per cell — 24 mirrored pairs (48 games) per (checkpoint, rung) cell.** The pair is §1's bootstrap resampling unit, so per-cell CI width shrinks ~1/√pairs and replicates resample *within* each cell; single-digit pairs make replicates degenerate, and `n ≈ 20–50` is the natural band, so 24 is chosen inside it. A per-checkpoint Elo pool then spans ≈12–15 cells (≈288–360 pairs). Stated honestly, the total-game arithmetic is: forms 5/6/7 × the four frozen network-free rungs 1–4 = 12 cells, plus at most 3 rung-8 cells (pin 5) = at most 15 cells/checkpoint; with `K` = 30, that is at most 30 × 15 × 24 × 2 = **21,600** eval games. Feasibility is measured by the M4 production-`S` benchmark (task 9) and governed by the liveness bound (pin 11, §12 M4) — it is never asserted here.
2. **Cell semantics.** Although §9 lists rungs 5–7 among the frozen ladder's entries, each is really a *checkpoint-parameterized agent form* rather than a fixed opponent — the reading M2.5 already froze via `evaluation.agent_form: "rung7_mcts_policy_value"`. Concretely: every member checkpoint `v` instantiates all three forms, each of which plays the four frozen network-free rungs 1–4; form 7 additionally meets the rung-7 forms of selected historical checkpoints under the rung-8 rule (pin 5), and it is this form's rating that §1 reports as `Elo_k`. One anchored Bradley–Terry fit spans every agent in play — all network-free rungs, all three forms of every member, every rung-8 opponent — connected through the rungs 1–4 they share. Neither the network-free roster nor the opening balancer is hardcoded into the harness: each game supplies both through its adapter's eval profile (Blokus: the M1.6 rungs 1–4 plus `start_square_balancer`, consistent with §12 M1.6's ownership split), which is what keeps the harness itself game-generic.
3. **Eval search-form definition.** Every form's move comes from a **fresh, deterministic search of exactly S simulations rooted at the current state**, run with no cross-move tree reuse — carrying visits forward would leave "S sims per move" ill-defined, and in any case the M1.6 `Agent` contract already forbids an agent from retaining state between calls. The root then plays argmax-N with no root noise, breaking ties toward the lowest action id; apart from the no-reuse rule, a form's per-move lifecycle mirrors rung 4's. Because these constants define the form, they are baked into its version string — `rung5-v1-<model_version>`, `rung6-v1-<model_version>`, `rung7-v1-<model_version>` — so changing any of them mints a new form version rather than editing one already in service.
4. **Rung-6/7 eval sim budget — S = 512 (gap-fill pin).** Nothing upstream of this amendment fixed a simulation count for the network rungs, yet a frozen rung's strength has to mean the same fixed budget indefinitely; this closes that gap at **S = 512**, matching D6's full tier (plies 0–1) and following the shape already set by M2.5's `evaluation.sims` field.
5. **Rung-8 historical-opponent rule.** Each member checkpoint `v` plays rung-7(`v`) vs. rung-7(`u`) for `u` ∈ {`v`−1, `v`−⌈`K`/4⌉, 1} ∩ [1, `v`−1], deduplicated — drawn from **members 1..`K` only** (`v0` is never an opponent, per §6.2's checkpoint/publish boundary rule (b)). With `K` = 30, `v`−⌈`K`/4⌉ = `v`−8. Bounded at **at most 3 cells/checkpoint**: `v`−1 keeps consecutive members Elo-connected, `v`−8 spans the mid-range, and `1` anchors every member back to the earliest.
6. **Finite-fit convention.** Both the point estimate and every bootstrap replicate go through the same fit: the M1.6 coordinate-ascent Bradley–Terry procedure, run with its **one virtual draw per unordered matchup** retained as the pre-registered regularizer — so §12 M1.6's scaffolding is *adopted* by this protocol rather than superseded, as it was described before this amendment. The regularizer is not optional bookkeeping: a late checkpoint shutting out rung 1 is exactly the outcome the project wants, that kind of complete separation is expected to appear even more often across bootstrap resamples, and without the virtual draw the unregularized MLE diverges to infinity — at which point `core/elo.py`'s bracket-expansion bisection (lines 106–110) has no finite bracket to find and never terminates.
7. **Bootstrap / Mann–Kendall conventions.** Δ̂ is reported as the statistic computed on the original sample, not a bootstrap mean; its confidence interval is the percentile interval taken over the replicate Δs, always via the same exact order-statistic rule — endpoints at ranks (`B`+1)·0.025 and (`B`+1)·0.975 — which restricts `B` to values making both ranks integers (`B` ≡ 39 mod 40: 39, 79, …, 1,999); a non-admissible `B` is rejected loudly rather than silently rounded. Production runs use **`B` = 1,999** (ranks 50/1,950, satisfying §1's "B ≈ 2,000"), and no other `B` may back an authoritative verdict. Each replicate `b` draws from a stream keyed off a single recorded `bootstrap_seed = derive_seed(eval_seed, PURPOSE_BOOTSTRAP)`. Separately, Mann–Kendall is always reported **two-sided**, with both the continuity and tie corrections applied to the normal approximation, and it never gates the verdict. Its degenerate cases are pinned rather than left to the implementation: fewer than 3 points yields an explicit insufficient-data result (no z, no p, no claim), and σ = 0 (every observation tied, forcing `S` = 0) yields z = 0, p = 1 so that no division-by-zero path exists. Both the fit and the trend test iterate in a fixed deterministic order — cells in sorted cell-id order, replicates by index. The gate itself is unchanged: CI strictly above 0.
8. **Provisional vs. authoritative reporting.** Δ̂, its CI, and the pass/fail gate exist only once the series is complete — the final ⌈`K`/3⌉ has no meaning before all `K` members are scored, so no partial "Δ" is ever computed or surfaced, even as an advisory number. Whatever is reported while a run is still in flight — per-checkpoint Elo and CIs, a Mann–Kendall reading over whatever prefix is currently defined, and progress metadata such as `checkpoints_evaluated`/`k_target` — must never be presented as the pre-registered contrast itself; that verdict belongs to §12 M5.5 alone.
9. **Analysis snapshot.** Every computation that depends on the accumulated evidence — a fit, a CI, a verdict, a plateau check — reads from one immutable snapshot rather than the live manifest: the longest contiguous run of members 1..k̂ whose cell sets are each fully registered and complete. Because a completed cell never changes afterward, taking a snapshot can never race the process still writing new results, an incomplete cell can never leak into an analysis, and the evidence fingerprint is well-defined as long as it is computed over that same snapshot and nothing more recent.
10. **Protocol version + fingerprint.** This amendment fixes `protocol_version = 1`. Every convention it covers — the form-version constants, seed derivation, the record schema, the finite-fit convention, the quantile and Mann–Kendall rules, iteration order, and the snapshot/gating semantics — has a code-side mirror collected into one constants registry, and that registry is hashed into a **protocol fingerprint** stamped onto eval configs, cell headers, and verdicts alike. Changing anything the fingerprint covers cannot be a silent edit: it necessarily produces a new protocol version and, with it, a **new eval namespace**, since the relaunch guard refuses to mix evidence stamped with different fingerprints.
11. **Operational liveness bound.** For "live and concurrent" (§12 M4) to be a falsifiable claim rather than an aspiration, this amendment fixes the numbers that would falsify it: a sustained eval lag — newest published member minus newest fully-scored member — of no more than **4 member checkpoints**, and no more than **5%** learner-throughput degradation attributable to the harness running alongside training. Both are measured during M5.5 against the M4 exit benchmark (pin 1's production-`S` bench). The bound is about liveness only, not correctness: the authoritative verdict still unconditionally requires the complete `K`-set, so breaching the lag bound means the harness catches up — running past the end of training if it must — never that a partial verdict is issued in its place.

The profiled-plateau rule's six constants (the window length, the Mann–Kendall significance level, the named windowed contrast, the CI-width threshold, the GPU-hour window, and the anti-flap confirmation clause) join this same protocol registry under pin 10, pinned doc-first at task 8; the full rule is stated in §12 M4, not repeated here.
...
The list is a topological order, not a strict serial chain; two branches run off the critical path and are called out inline (Othello depends only on the M0 interface; the M1.6 baseline opponents depend only on the M1 rules engine). Earlier revisions marked open scalars with a "to pin doc-first" flag — naming a scalar left unpinned by D1–D12 that had to be fixed in the doc before its owning milestone could build against it. All three flagged scalars (`λ_aux`, the publish-interval/`K` pin, and pairs-per-cell — status header) are now pinned, so no such flag remains anywhere in this milestone list.
...
- **M1.5 — Abstraction test (firm): Othello.** *(Depends only on the M0 `Game` interface, not on any Blokus code — schedule in parallel with M1 rather than strictly after it; retiring abstraction leakage early is cheaper than after M1 has hardened the interface.)* Acceptance: implementing it touches only `games/` + `configs/` — **zero `core/` changes**. Beyond the diff check, it must exercise what Blokus can't:
  ...
  - **Convention pins (M1.5, Othello) — frozen into the adapter, encoding surface, and symmetry table:**
    - **Coordinates & flatten:** 0-indexed `(r, c)`, row-major, 8×8; `action_id = r*8 + c` for placements (ids 0–63), **pass = 64**; `policy_shape = (65,)` (flat head, per this milestone's 64+1 pin).
    - **Players & start:** player 0 = Black and moves first; standard initial position (0-indexed): White (3,3),(4,4); Black (3,4),(4,3).
    - **Pass & termination:** pass is legal **iff** the mover has no placement and the position is nonterminal; a position is terminal **iff** neither player has a placement. `apply` always hands the move to the opponent (strict alternation at the action level — the *placement* sequence is what goes non-monotone/consecutive).
    - **Input planes (M1.5-carried encoding):** 2 mover-relative occupancy planes (own, opponent) as nested tuples, `T=1`, `input_planes = 2` — consistent with §5.2's own/opponent convention (side-to-move plane constant under relabeling). The `symmetry_group` plane-transform slot is **real** at M1.5 (Blokus's stays an M2 sentinel).
    - **D4 order & orientation:** `(identity, rot90, rot180, rot270, flip_h, flip_v, diag, antidiag)` with **rot90 = clockwise** (`(r,c) → (c, 7−r)`), `flip_h` mirroring columns, `diag` the main-diagonal transpose.
    - **Value target:** `z = sign(disc_diff)` over raw disc counts (empties unassigned — the sign, incl. draws, is invariant to the empties-to-winner convention); **no aux head** (the score-diff aux is a Blokus D8 lever, not a capability of this adapter).
...
- **M1.6 — Baseline opponents + game runner (NEW; absolute-strength anchor):** the network-free ladder rungs of §9 — (1) uniform random, (2) largest-piece/coverage heuristic, (3) mobility heuristic — plus the **paired/mirrored game runner** (alternate first player, balance the start-point choice, draws scored 0.5) and the anchored-Elo scaffolding (rung 1 fixed at 0). Depends only on M1's rules engine; gives M2.5/M3 a cheap "does self-play beat random yet?" measurement before the full M4 harness exists. **Rung 4 (UCT + random rollouts) is *not* here** — it needs the M0 search engine wired to a rollout evaluator and lands with M3. Network rungs 5–8 and the §1 statistical protocol stay in M4, which extends this runner/scaffolding.
  - **Convention pins (M1.6) — the frozen operational definitions of the network-free rungs, the pair protocol, and the Elo fit:**
    ...
    - **Rung 3 (mobility, interface-generic):** 1-ply greedy over successors `s' = apply(s, a)`, scored on a four-band total order (`terminal_utility` takes priority over mobility): a **terminal win** outranks every nonterminal successor; a **terminal loss** ranks below every one; a **terminal draw** ranks below all nonterminal successors but above a terminal loss (so a live position is preferred to locking an immediate tie — in Blokus, playing on almost always means placing more pieces and improving score, and forfeiting winnable games to weaker opponents would only depress the anchor's Elo). Nonterminal `s'` scores `−|legal(s')|` when the next mover is the opponent, `+|legal(s')|` when the mover moves again (opponent blocked/skipped — only observable through the interface's `current_player`). Uniform-random among ties (seeded). Lives in `core/` (uses only the `Game` interface).
...
- **M3 — Self-play baseline (functional / correctness; fixed-sim):** actor + replay + learner on one GPU at a **fixed 128 sims** (the D6 validate tier — *not* the PCR schedule, which is M5). Scope is correctness and end-to-end wiring, not throughput.
  ...
  - **Rung 4 (UCT + random rollouts)** wired here (needs the M0 search engine + a rollout evaluator); registered as a frozen ladder opponent alongside the M1.6 rungs — its own standalone module (`core/uct.py`), not a configuration of the PUCT engine.
    - **Convention pin (M3):** UCB1 selection `Q(a) + c·√(ln ΣN / N(a))` with `c = √2` over the parent's total child visits; untried children are visited once before any UCB1 comparison, in lowest-action-id order; ties (untried and UCB1 alike) break by lowest action id. Rollout: uniform-random over `legal_moves` to a terminal. Backup: the same edge-relative player-aware convention as `core/mcts.py` (§6.2), generalized through `terminal_utility` so non-alternating pass games are handled correctly. Move choice: most visits, ties by lowest action id (mirrors `MCTS.best_action`). Frozen at **1,000 simulations/move**, registered under identity `uct-rollout-v1`; any future change to these constants is a new rung version, never an edit of the frozen one.
...
- **M4 — Evaluation harness (stood up before the production run and run *concurrently* with it):** the frozen-ladder network rungs — (5) network policy, no search; (6) uniform-prior MCTS with network value (the M0 uniform-prior config flag); (7) full policy-and-value MCTS; (8) historical checkpoints — plus the §1 statistical protocol (paired bootstrap primary; Mann–Kendall secondary; per-checkpoint CIs), extending the M1.6 runner/scaffolding. The full protocol — pairs-per-cell, cell semantics, the eval search-form + sim budget, the rung-8 rule, the finite-fit convention, and the bootstrap/Mann–Kendall conventions — is pre-registered doc-first in §9's eleven pins, before any evaluation game exists.
  - **Entry / coupling condition (new):** M4 is stood up **before** the M5.5 production run and runs concurrently with it (below), so it depends on M3 artifacts existing first: the publish-interval/`K` pin with `v0` excluded from the `K`-member set (§6.2), the checkpoint schema with orientation-hash validate-on-load, the metrics reducer's per-version `(learner_step, positions_evaluated, gpu_hours)` join on the `checkpoint_published` marker, the run entrypoint, and ladder rung 4.
  - **Coupled to the run, not strictly downstream:** the §1 Δ contrast needs checkpoints scored (or their mirrored-pair games logged at per-(checkpoint, rung) granularity) *as they are produced*, so the harness + its paired-result logging schema must be live from the first production checkpoint — otherwise the early-third checkpoints are unscored/incomparable and Δ is unreconstructable without a re-run.
  - **Before loading historical checkpoints (rung 8):** assert the orientation-table hash matches (the M3 read-side path).
  - **Plateau-detection rule (operationalize the M6 gate) — the committed rule, replacing the earlier "e.g." sketch:** "profiled plateau" is quantitative and falsifiable, decidable **during** the run from the analysis snapshot's artifacts alone (pin 9's snapshot semantics — nothing post-hoc, nothing partial), so every M6 lever go/no-go and the ceiling declaration (§13) rests on it. Because the rule gates the M6 levers' irreversible decisions, the anti-flap confirmation clause below is part of the rule's definition, not an implementation nicety — and because the levers themselves stay human calls, the detector only reports a tri-state verdict, never triggers anything. Six constants, pinned together as one predicate (task 8):
    - **Window — `M` = 8 evaluated member checkpoints.** About a quarter of `K` = 30: long enough for the trend test below to have real power at its significance level, short enough to answer "has progress stopped *recently*" rather than "on average over the whole run."
    - **Trend test — Mann–Kendall at `α` = 0.05, two-sided**, run over the window's `M` checkpoints (reusing §9 pin 7's implementation and degenerate-case pins verbatim); non-significant means `p ≥ 0.05`, the same 95% convention §1 and §9 already use throughout, so the plateau rule introduces no second confidence level.
    - **A named windowed contrast, not the §1 Δ:** `Δ_window = mean(Elo over the newest ⌈M/2⌉ window members) − mean(Elo over the oldest ⌈M/2⌉ window members)`, computed on the point estimate with its CI taken over the *same* bootstrap replicates via the *same* admissible-`B` order-statistic rank rule as pin 7. This is explicitly **not** §1's Δ, which is defined only once, over the complete `K`-set (pin 8), and never exists mid-run — the earlier "Δ-CI width" sketch wording is retired in favor of this named statistic to end that ambiguity.
    - **CI-width threshold — strictly below 75 Elo points.** An order of magnitude below the Elo gains typical of the early run, so "flat" means flat relative to real signal rather than an artifact of a loose interval; achievable at the pin-1 evidence density (≈288–360 pairs/checkpoint) without a special-cased evaluation budget.
    - **GPU-hour window — the `M` window members must span ≥ 8 GPU-hours** of single-counted orchestrator GPU time (the §1 x-axis join; §12 M3's observability counters), so a plateau can never be declared over a burst of cheap, closely-spaced checkpoints.
    - **Anti-flap confirmation — two consecutive evaluated member checkpoints.** PLATEAU is declared only when the full conjunction — MK non-significant **AND** CI width < 75 **AND** GPU-hour span ≥ 8 — holds at two consecutive snapshots whose newest window members are themselves consecutive versions; a single satisfying snapshot is NO-PLATEAU, pending confirmation.
    - **Insufficient-data clause (first-class, not a plateau):** a window shorter than `M`, a Mann–Kendall insufficient-data result, **an all-tied window** — every point estimate in the window bit-identical, which forces Mann–Kendall's own pinned σ = 0 degenerate branch (`p` = 1 rather than an explicit insufficient-data result) and so carries no real trend information despite reporting as if non-significant — or a window member missing its GPU-hour coordinate is **INSUFFICIENT-DATA** — a third tri-state outcome, never coerced into NO-PLATEAU or PLATEAU. All six constants above join the §9 pin-10 protocol registry additively: changing any one of them is a protocol-version bump, exactly like every other registry entry.
  - **Bootstrap seed:** the B ≈ 2,000-replicate resample carries its own recorded seed, `bootstrap_seed = derive_seed(eval_seed, PURPOSE_BOOTSTRAP)` (§9 pin 7). Pairs-per-cell and every other protocol constant are pinned in §9, not here.
  - **Operational liveness bound (new, §9 pin 11):** a maximum sustained eval lag of 4 member checkpoints (newest published member minus newest fully-scored member) and a maximum 5% learner-throughput degradation while the harness runs, both measured during M5.5 against the M4 exit benchmark, make "live and concurrent" falsifiable; a breach means catch-up, the harness running past training's end, never a partial verdict.
...
- **M5.5 — Production run + primary-criterion verdict (NEW; the actual experiment):** run the M5-efficient, PCR-scheduled self-play loop to accumulate the *K* version-pinned checkpoints (§6.2), with M4's eval + logging live throughout; then compute the §1 Δ paired-bootstrap contrast and the Mann–Kendall secondary and render the **pre-registered verdict** (Δ 95% CI strictly above 0). The M6 levers interleave with this run — each pulled only on an M4-defined profiled plateau. *(Pinned upstream at M3, §6.2: weight-publish interval **200 learner steps**; checkpoint count **K = 30**.)*
