...
Unpinned scalars flagged doc-first: mirrored pairs per (checkpoint,rung) cell (M4/§9). Already pinned at its milestone: aux loss weight **λ_aux = 0.25** (M2, §7); weight-publish interval **200 learner steps** / checkpoint count **K = 30** (M3, §6.2).
...
- **W&B mirror (optional, auxiliary tooling — issue #90):** `python3 -m pip install -e ".[wandb]"` then `python3 scripts/wandb_sync.py <run_dir> --project <name> [--follow] [--finalize]` mirrors an existing run dir's `core.observability` metrics into Weights & Biases. Idempotent via an explicit, deterministic per-row `step` (never wandb's implicit step, reserved for exactly this) plus `resume="allow"`: replaying an already-accepted row is a server-side no-op, not merely bounded (mechanism verified against the `wandb` SDK source; full guarantee and its assumptions in the script's module docstring). Trailing/still-in-flight flush groups are held back by default (one-shot and `--follow` alike) — pass `--finalize` only once the run directory is known complete to flush them; multi-actor throughput is buffered and released behind a cross-process watermark so the merged timeline never regresses. Read-only over the run dir except for its own `.wandb_sync_state.json` sidecar; `core/` never imports `wandb`, and the full battery passes without the extra installed.

*(Fill in as tooling lands: eval harness.)*
