...
- **W&B mirror (optional, auxiliary tooling — issue #90):** `python3 -m pip install -e ".[wandb]"` then `python3 scripts/wandb_sync.py <run_dir> --project <name> [--follow]` mirrors an existing run dir's `core.observability` metrics into Weights & Biases, idempotently (re-syncing never duplicates history). Read-only over the run dir except for its own `.wandb_sync_state.json` sidecar; `core/` never imports `wandb`, and the full battery passes without the extra installed.
